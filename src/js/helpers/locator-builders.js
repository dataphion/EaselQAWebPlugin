...
import { MozillaBrowserBot } from "../content/selenium-browserbot";
import { bot, core } from "../content/atoms";

export function LocatorBuilders(window) {
  this.window = window;
...
LocatorBuilders.prototype.buildAll = function(el) {
  console.log(" builder element --------->", el);
  console.log("previous ele ---------->", el.previousSibling);
  ...
  console.log("full xpaths ---->", full_xpath[0]);
  locators.push(full_xpath);

  // get Grid/table details---
  let selected_grid_data = {};
  if (full_xpath[0].includes("table")) {
    selected_grid_data["grid"] = true;
    selected_grid_data["row_data"] = [];

    let path = full_xpath[0].split("/td");
  ...
  for (var i = 0; i < LocatorBuilders.order.length; i++) {
    ...
    try {
      ...
      console.log("buildWithResults ----------->", buildWithResults);
  ...
  let localtors_data = {
    locators: locators,
    selected_grid_data: selected_grid_data
  };
  return localtors_data;
};
...
LocatorBuilders.prototype.getCSSSubPath = function(e) {
  ...
  for (var i = 0; i < css_attributes.length; i++) {
    ...
    if (value) {
      ...
      if (attr == "class") return e.nodeName.toLowerCase() + "." + value.replace(/\s+/g, ".").replace("..", ".");
      return e.nodeName.toLowerCase() + "[" + attr + '="' + value + '"]';
    }
  }
  if (this.getNodeNbr(e)) return e.nodeName.toLowerCase() + ":nth-of-type(" + this.getNodeNbr(e) + ")";
...
LocatorBuilders.add("ui", function(pageElement) {
  return UIMap.getInstance().getUISpecifierString(pageElement, this.window.document);
});
...
LocatorBuilders.add("dom:name", function(e) {
  if (e.form && e.name) {
    ...
    if (formLocator) {
      var candidates = [formLocator + "." + e.name, formLocator + ".elements['" + e.name + "']"];
  ...
  while (current != null) {
    if (current.parentNode != null) {
      ...
      if (
        ...
        return this.preciseXPath("//" + this.xpathHtmlElement(current.parentNode.nodeName.toLowerCase()) + "[@id=" + this.attributeValue(current.parentNode.getAttribute("id")) + "]" + path, e);
      }
    } else {
      return null;
    }
